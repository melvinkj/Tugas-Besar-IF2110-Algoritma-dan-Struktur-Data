...
int main()
{
    ...
    if (validate)
    {
        ...
        POINT test = LocateS(peta);

        while (running_state)
        {
            // Validate every command
            printf("%s di posisi: (%d,%d)\n", nama.content, test.X, test.Y);
            printf("Waktu : \n");
            printf("Notifikasi : -\n");
            displayMatrix(peta);
            printf("\n");
            ...
            // testing new func
            processCommand(input, &test, &peta);
            // displayMatrix(peta);
            printf("Posisi S Sumbu : (%d,%d)\n", test.X, test.Y);
            if (checkAdjacent('T', peta, test)){
                printf("ada T di sekitar S!\n");
            } else {
                printf("Tidak bisa T\n");
            }
            if (checkAdjacent('M', peta, test)){
                printf("ada M di sekitar S!\n");
            }
            else {
                printf("Tidak bisa M\n");
            }
            if (checkAdjacent('C', peta, test)){
                printf("ada C di sekitar S!\n");
            }
            else {
                printf("Tidak bisa C\n");
            }
            if (checkAdjacent('F', peta, test)){
                printf("ada F di sekitar S!\n");
            }
            else {
                printf("Tidak bisa F\n");
            }
            if (checkAdjacent('B', peta, test)){
                printf("ada B di sekitar S!\n");
            }
            else {
                printf("Tidak bisa B\n");
            }
